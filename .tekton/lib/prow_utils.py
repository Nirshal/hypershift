...
__all__ = ["DEFAULT_PREFIX", "short_name", "trigger_prow_job",
           "resolve_prow_url", "get_prow_job_status"]


DEFAULT_PREFIX = "periodic-ci-openshift-hypershift-release-5.0-periodics-"


def short_name(job_name, prefix=DEFAULT_PREFIX):
    """Strip common prefix from a Prow job name for display.

    Args:
        job_name: Full Prow periodic job name.
        prefix: Prefix string to remove. Defaults to the standard
            HyperShift periodic job prefix.

    Returns:
        Shortened job name, or the original name if the prefix
        does not match.
    """
